...
import { Topbar, Btn, Icon, Field, SelectField, Card, Spinner, ErrorState } from '@/shared/ui'
...
export default function CircuitDiagram() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      api.panels.list(),
      api.inverters.list(),
      id ? api.projects.get(Number(id)) : Promise.resolve(null),
    ])
      .then(([tmpls, panels, inverters, proj]) => {
        if (!alive) return
        setTemplates(tmpls)
        if (proj) {
          const panel = panels.find((p) => p.id === proj.panel_id) || null
          const inverter = inverters.find((i) => i.id === proj.inverter_id) || null
          const battery = !!proj.battery_id
          setHasBattery(battery)
          setTemplate(solarTemplateFor(true, battery))
          setForm((f) => ({
            ...f,
  ...
  if (loading) {
    return (<><Topbar title={t('title')} crumb={t('crumb')} /><div className="sun-content"><Spinner label={t('preview.loading')} /></div></>)
  }
  if (loadError) {
    return (<><Topbar title={t('title')} crumb={t('crumb')} /><div className="sun-content"><ErrorState message={loadError} onRetry={() => nav(0)} /></div></>)
  }
  ...
  return (
    <>
      <Topbar
        title={t('title')}
        crumb={t('crumb')}
        actions={<Btn variant="primary" icon="download" onClick={downloadSvg}>{t('actions.download')}</Btn>}
      />
      <div className="sun-content">
        ...
        <div className="sun-circuit">
          ...
          <Card className="sun-circuit__preview">
            <div className="sun-divider">{t('preview.title')}</div>
